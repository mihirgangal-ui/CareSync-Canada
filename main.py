...
def load_data():
    default_data = {
        ...
        "settings": {"senior_name": "", "caregiver_name": "", "caregiver_email": "", "user_role": ""}
    }
    ...
    with tab1:
        ...
        if st.button("Log In"):
            if data["settings"]["caregiver_email"] == user_email and user_email != "":
                ...
                # Auto-set the role based on what they picked during signup
    ...
    with tab2:
        ...
        with st.form("signup_form"):
            ...
            with col2:
                ...
                # --- NEW ROLE CAPTURE ---
                u_role = st.selectbox("I am signing up as a:", ["Caregiver", "Senior"])
            ...
            if st.form_submit_button("Complete Intake & Sign Up"):
                if s_name and c_name and c_email:
                    ...
                    save_data(data)
                    st.success(f"Account created for {u_role}! You can now switch to the 'Sign In' tab.")
                else:
...
# --- INSIDE THE GATE ---
    ...
    # We use the role saved in session state (derived from their signup choice)
    ...
    # --- FEATURE: SENIOR VIEW ---
        ...
        st.write(f"Logged in as: {data['settings']['user_role']}")
        mood = st.select_slider("How are you feeling today?", options=["Low", "Ok", "Good", "Great"])
        ...
        # Option for the Senior to Logout
        if st.button("Logout"):
            st.session_state.authenticated = False
            st.rerun()
    ...
    # Sidebar Logout (for Caregiver only)
    if st.session_state.role == "Caregiver":
        st.sidebar.markdown("---")
