...
if not st.session_state.session_user:
    ...
    with tab2:
        with st.form("signup"):
            ...
            u_email = st.text_input("My Email")
            # Mandatory Mobile + Optional Caregiver Tagging
            u_mob = st.text_input("Mobile Number (Required) *")
            cg_tag_email = ""
            if u_role == "Senior":
                cg_tag_email = st.text_input("Caregiver's Email (to auto-link)")
            
            if st.form_submit_button("Register"):
                if u_email and u_name and u_mob: # Validation Check
                    ...
                    if u_role == "Caregiver":
                        ...
                        # AUTO-TAGGING LOGIC
                        if cg_tag_email:
                            if cg_tag_email in db["links"]:
                                db["links"][cg_tag_email].append(s_id)
                                st.success(f"Linked to caregiver: {cg_tag_email}")
                            else:
                                st.warning("Caregiver email not found. You can link later!")
        ...
        if not my_seniors: st.info("No seniors managed yet.")
            ...
            for idx, sid in enumerate(my_seniors):
                ...
                with cols[idx % 3]:
                    ...
                    if st.button(f"Open Records: {sdata['name']}", key=sid):
        ...
        if st.session_state.active_senior_id:
            ...
            with t_med:
                with st.form("med_form"):
                    m_n = st.text_input("Med Name")
                    m_f = st.text_input("Frequency")
            ...
            with t_cal:
                with st.form("cal_form"):
                    ev = st.text_input("Event"); l = st.text_input("Location")
